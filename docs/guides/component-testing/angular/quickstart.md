...
```ts title=src/app/stepper/stepper.component.ts
...
```js title=src/app/stepper/stepper.component.cy.ts
...
```ts title=src/app/stepper/stepper.component.cy.ts
it('stepper should default to 0', () => {
  cy.mount(StepperComponent)
...
options. See the
[MountOptions API](/guides/component-testing/angular/api#MountOptions) for
